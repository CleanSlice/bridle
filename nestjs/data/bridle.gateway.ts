...
interface IPendingSync {
  resolve: (value: IBridleSyncAgentResult) => void
...
export class BridleGateway extends IBridleGateway {
  ...
  /** Browser clients: clientId → { agentId, send, isAdmin } */
  ...
  registerClient(
    ...
    this.logger.log(
      `Browser client registered: ${clientId} agentId=${agentId} admin=${isAdmin}${prompt ? ' +prompt' : ''}${capabilities?.length ? ` caps=[${capabilities.join(',')}]` : ''} (total: ${this.clients.size})`,
    )
  }
  ...
  sendToAgent(clientId: string, agentId: string, text: string, parts: BridlePart[]): void {
    ...
    agentSend({
      ...
      ...(client?.capabilities?.length ? { capabilities: client.capabilities } : {}),
      messageId: randomUUID(),
    })
  }
  ...
  setDebug(agentId: string, enabled: boolean): void {
    const agentSend = this.agents.get(agentId)
    if (!agentSend) {
      this.logger.debug(`setDebug skipped: agent not connected for agentId=${agentId}`)
      return
    }
  ...
  clearAgentSession(agentId: string, channel: string): void {
    ...
    this.logger.log(`Pushed session_clear for channel=${channel} to agent agentId=${agentId}`)
  }
  ...
  handleDebugEvent(agentId: string, data: IBridleDebugEvent): void {
    ...
    if (delivered === 0) {
      this.logger.debug(`Debug event dropped: no admin clients for agentId=${agentId}`)
    }
  }
  ...
  syncAgent(
  ...
  ): Promise<IBridleSyncAgentResult> {
    const agentSend = this.agents.get(agentId)
    if (!agentSend) {
    ...
    return new Promise<IBridleSyncAgentResult>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pendingSyncs.delete(requestId)
        reject(new Error(`Sync timed out after ${timeoutMs}ms (agentId=${agentId})`))
      }, timeoutMs)
