...
export abstract class IBridleGateway {
   ...
   * if the agent for botId isn't currently connected — the next register
   * handshake should re-send the desired value.
   */
  abstract setDebug(botId: string, enabled: boolean): void
  /**
   * Ask the agent for botId to push its local state to remote storage and
   * resolve when the agent acks. Resolves with `agentOnline=false` immediately
   * if no agent is connected for botId.
   */
  abstract syncAgent(botId: string, timeoutMs?: number): Promise<IBridleSyncAgentResult>
