...
- `agents: Map<botId, sendFn>` — each agent runtime registers under its `botId`.
- `clients: Map<clientId, { botId, sendFn, isAdmin }>` — each connected browser.

When a browser sends a message, the hub looks up the agent for that `botId` and forwards. When the agent responds, the hub looks up the originating `clientId` and routes back. That's it.

## Per-bot isolation

Every connection — browser or agent — declares a `botId` in the WebSocket auth handshake. The hub enforces:

- Browsers only receive responses from the agent serving their `botId`.
- Agents only see messages from browsers asking for their `botId`.
- Multiple bots can serve different audiences through the same hub.

```
Bot A (botId: "bot-a")     Hub      Browser 1 (botId: "bot-a")
Bot B (botId: "bot-b")     Hub      Browser 2 (botId: "bot-b")
```
...
- **Agents** authenticate with a shared API key (`BRIDLE_API_KEY`) plus their declared `botId`.
- **Browsers** authenticate with a JWT signed by your app, plus the `botId` they want to chat with.
